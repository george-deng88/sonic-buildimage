/*
 * switchboard_fpga.c - driver for ds4100 Switch board FPGA/CPLD.
 *
...
#define CLASS_NAME "ds4100_fpga"
#define DRIVER_NAME "switchboard"
#define FPGA_PCI_NAME "ds4100_fpga_pci"
...
static struct platform_device *ds4100_dev;
...
struct ds4100_fpga_data *fpga_data;
...
static struct device * ds4100_sff_init(int portid) {
...
static const struct i2c_algorithm ds4100_i2c_algorithm = {
 ...
 */
static struct i2c_adapter * ds4100_i2c_init(struct platform_device *pdev,
        int portid, int bus_number_offset)
{
    ...
    new_adapter->algo  = &ds4100_i2c_algorithm;
...
static int ds4100_drv_probe(struct platform_device *pdev)
{
    int ret = 0;
    int portid_count;
    ...
    fpga_data = devm_kzalloc(&pdev->dev, sizeof(struct ds4100_fpga_data),
                             GFP_KERNEL);
    ...
    for (portid_count = 0 ; portid_count < VIRTUAL_I2C_PORT_LENGTH ; portid_count++) {
        fpga_data->i2c_adapter[portid_count] = ds4100_i2c_init(pdev, portid_count, VIRTUAL_I2C_BUS_OFFSET);
    }
    ...
    for (portid_count = 0; portid_count < SFF_PORT_TOTAL; portid_count++) {
        ...
        if (i2c_adap) {
            fpga_data->sff_devices[portid_count] = ds4100_sff_init(portid_count);
...
static int ds4100_drv_remove(struct platform_device *pdev)
{
    int portid_count;
...
static struct platform_driver ds4100_drv = {
    .probe  = ds4100_drv_probe,
    .remove = __exit_p(ds4100_drv_remove),
    .driver = {
        .name   = DRIVER_NAME,
    },
};
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    ds4100_dev = platform_device_register_simple(DRIVER_NAME, -1, NULL, 0);
    platform_driver_register(&ds4100_drv);
    return 0;
...
{
    platform_driver_unregister(&ds4100_drv);
    platform_device_unregister(ds4100_dev);
    fpgafw_exit();
    pci_iounmap(pdev, fpga_dev.data_base_addr);
    pci_release_regions(pdev);
    pci_disable_device(pdev);
...
void ds4100_exit(void)
{
    pci_unregister_driver(&pci_dev_ops);
}

module_init(ds4100_init);
module_exit(ds4100_exit);

MODULE_AUTHOR("Pradchaya P. <pphuhcar@celestica.com>");
MODULE_DESCRIPTION("Celestica ds4100 switchboard driver");
